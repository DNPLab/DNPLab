...
# lets perform some basic processing. simply pass the entire workspace, 'ws', to the functions and the ws["proc"] will be manipulated. For example:
dnp.dnpNMR.remove_offset(ws)
dnp.dnpNMR.window(ws, linewidth=10)
dnp.dnpNMR.fourier_transform(ws, zero_fill_factor=2)
# this procedure removes the DC offset, multiplies by a window function, and fourier transforms.
...
# pro tip: default order is zeroth, first order is also available and is demonstrated below
...
# this time use a first order phase correction where the phase angle changes ('delta') linearly by pi/16 over the spectral range and pivots 100 points into the spectrum
...
# pro tip: by default the entire spectrum is integrated, be sure to optimize the width of the center and width of the integration window. For example,

# so lets integrate only over a certain range instead. First return to the data as it was before integrating by copying the saved 'spec'' back to 'proc'
...
# fit the integrals to an inversion recovery function to extract T1 using fit type="T1". Fit data are added to the object as the key 'fit'.
fit(ws_2D, type="T1")
