...
def deriveF(f, dx, deriv=4):
    ...
    try:
        ...
        raise NotImplementedError("only the following derivatives orders are implemented: {0}, but {1} was selected".format(cStencil.keys(),deriv))
...
def autophase(data, dim="f2",pivot=False ,deriv=1, gamma=1e-5):
    ...
    if pivot == False:
        ...
        raw_data=raw_data.flat[:size] #now this is not necessarily true if the dimension is not the first ?
    else:
    ...
    xopt, fopt,iter,funcalls,warnflags= scipy.optimize.fmin(
        optimfun, [0, 0], args=(raw_data, deriv, gamma, dx), disp= False, full_output=True
    )
    if warnflags==1:
        warnings.warn("Optimization reached maximum number of function evaluations, solution might not be optimal!")
    if warnflags==2:
        warnings.warn("Optimization reached maximum number of iterations, solution might not be optimal!")
    ph0, ph1 = xopt
    import phase
...
if __name__ == "__main__":
    ...
    plt.plot(np.real(phasedData4).values[:,0],linestyle='--',label='dnplab - order 4')
    plt.gca().set_title('nmrglue & dnplab')

    plt.legend()

    plt.figure()
    plt.plot((data_real-dnp1)/dnp1,linestyle='--',label='(nmrglue - dnplab order 1)/dnplab order 1')
    plt.plot((data_real-dnp4)/dnp4,linestyle='--',label='(nmrglue - dnplab order 4)/dnplab order 1')
