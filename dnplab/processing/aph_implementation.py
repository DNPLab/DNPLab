...
def autophase(data, dim="f2", pivot=False, deriv=1, gamma=1e-5,verbose=True):
    """
    ...
    """
    if verbose:
            data.add_proc_attrs("autophase", {"pivot": pivot, "deriv": deriv, "dim": dim,"phasetuples":[]})
        else:
    ...
    if pivot == False:
        ...
        for k in range(n_spectra):
            spectrum = data[dim,k]
            ph0,ph1 = _autophase(spectrum,deriv,gamma)
            phasedSpectrum = phase.phase(data, dim=dim, p0=ph0 / 2 / math.pi * 360, p1=ph1 / 2 / math.pi * 360)
            data[dim,k] = phased_spectrum.values
            if verbose:
                data._attrs["autophase"]["phasetuples"].append((ph0,ph1))
        data.fold
    else:
        ...
        try:
            ...
            raise ValueError("Pivot must be False or a tuple (dim2,element) where dim2 !=dim, you provided {0}".format(pivot))
        if dim_2 == dim:
            raise ValueError("dim2({0}) must not be dim({1})".format(dim_2,dim))
        pivot = data[dim,:,dim_2,element]
        pivot = autophase(dim,deriv=deriv,gamma=gamma,verbose=True)
        ph0,ph1 = pivot.attrs["autophase","phasetuples"][0]
        data.unfold(dim)
        data = phase.phase( data, dim=dim, p0=ph0 / 2 / math.pi * 360, p1=ph1 / 2 / math.pi * 360 )
        if verbose:
                data._attrs["autophase"]["phasetuples"].append((ph0,ph1))
        data.fold()
    return data
...
def _autophase(data,deriv,gamma):
    # assume that data is always a "1d" data object which can be used by raw_data=data.values.ravel()
