...
def download(names: str | list[str], url: str, apiKey: str | None = None, verify: bool = False):
    """
    Download dataset from LOGS
    ...
    if datasets.count != len(names):
        raise ValueError("The number of datasets found does not match the number of names provided.")
    ...
    file_list = [os.path.join(path, file) for file in os.listdir(path) if path_exten in file]
