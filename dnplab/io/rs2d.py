...
def import_rs2d(path,*args,datafile="data.dat", headerfile="header.xml", **kwargs):
    ...
    if path.suffix != ".xml":
        _warnings.warn("import_rs2d: got file that does not end in .xml, will try to open {} and try to get data.dat".format(str(path)))
    ...
    for child in root:
        ...
        for ind,entry in enumerate(child):
            try:
                ...
                attr[child.tag].__setitem__(key,value)
            except AttributeError as e:
                _warnings.warn("Error in finding key or value at entry {}, skipping entry without elements, attribute error: {}".format(ind,e))
    return attr
...
def _load_rs2d_data(path, attrs, **kwargs):
    ...
    dimNames = list( reversed( [ "ACQUISITION_MATRIX_DIMENSION_"+str(k) + "D" for k in range(1,5) ] + ["RECEIVER_COUNT"] ) )
    dims = list( reversed( [ "t"+str(k) for k in range(len(dimNames)) ] ) )
    dimValues = [ int( attrs['params'].get(k,1) ) for k  in dimNames]

    data = _np.reshape(data, dimValues)

    coords = [ _np.arange(k) for k in dimValues ]
    try:
        coords[-1] = coords[-1] * float(attrs.get('params',1).get('DWELL_TIME',1))
    except:
        pass
