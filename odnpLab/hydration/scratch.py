...
def calcODNP(Ep: np.array, T1p: callable):
    ...
    slC = 200e-6 # (Eq. 1-2) unit is M, spin label concentration for scaling relaxations to get "relaxivities"
    ...
    omega_e = 1.76085963023e11*(field/1000) # gamma_e is from NIST, field is converted to T. The field cancels jin the following ratio but you need these individually for the spectral density function later. unit is Hz
    omega_H = 2.6752218744e8*(field/1000) # gamma_H is from NIST, field is converted to T. The field cancels in the following ratio but you need these individually for the spectral density function later. unit is Hz
    wRatio = omega_e/omega_H # (Eq. 4-6) ratio of omega_e and omega_H, ~= S_0/I_0
    
    # Ep will be the series of enhancements
    # T1p will be the series of T1s after interpolated to match the enhancement series. Power not needed anymore, it is only used to line up and interpolate the T1s to the Enhancements
    
    ksig_smax = (1-Ep)/(slC*wRatio*T1p) # (Eq. 42) this calculates the series of k_sigma*s(p) which approximates to k_sigma*s_max
    k_sigma = max(ksig_smax)/s_max # (Eq. 43) this takes the maximum of the k_sigma*s(p) series and divides by the s_max to isolate k_sigma, unit is s^-1 M^-1
    
    ksig_bulk = 95.4 # unit is s^-1 M^-1 The only place I can find this is Franck, JM, et. al.; "Anomalously Rapid Hydration Water Diffusion Dynamics Near DNA Surfaces" J. Am. Chem. Soc. 2015, 137, 12013−12023. Figure 3 caption
    ...
    tcorr = getTcorr(ksi,omega_e,omega_H) # (Eq. 21-23) this calls the fit to the spectral density functions. The fit optimizes the value of tcorr in the calculation of ksi, the correct tcorr is the one for which the calculation of ksi from the spectral density functions matches the ksi found experimentally. tcorr unit is ps
    
    tcorr_bulk = 54 # (section 2.5), "corrected" bulk tcorr, unit is ps
    dH2O = 2.3e-9 # (Eq. 19-20) bulk water diffusivity, unit is d^2/s where d is distance in meters. *didnt use m to avoid confusion with mass
    dSL = 4.1e-10 # (Eq. 19-20) spin label diffusivity, unit is d^2/s where d is distance in meters. *didnt use m to avoid confusion with mass
    dLocal = (tcorr_bulk/tcorr)*(dH2O+dSL) # (Eq. 19-20) local diffusivity, i.e. diffusivity of the water near the spin label
    
    ############################################################################
    # This is defined in its most compact form in:
    # Frank, JM and Han, SI;  Chapter Five - Overhauser Dynamic Nuclear Polarization for the Study of Hydration Dynamics, Explained. Methods in Enzymology, Volume 615, 2019
    ...
    k_low = ((5*k_rho)-(7*k_sigma))/3 # section 6, (Eq. 13). this describes the relatively slowly diffusing water near the spin label, sometimes called "bound" water
    ############################################################################
    
    klow_bulk = 366 # unit is s^-1 M^-1 The only place I can find this is Franck, JM, et. al.; "Anomalously Rapid Hydration Water Diffusion Dynamics Near DNA Surfaces" J. Am. Chem. Soc. 2015, 137, 12013−12023. Figure 3 caption
